...
export default function SignupPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!username.match(/^[a-z0-9._]{3,24}$/)) {
      setErr('Foydalanuvchi nomi: 3-24 belgi, kichik harflar, raqamlar, nuqta, pastki chiziq.');
      return;
    }
  ...
  return (
    <div className="card" style={{ padding: 28 }}>
      <h1 style={{ fontFamily: 'var(--font-geist)', fontSize: 26, fontWeight: 700, color: ink,
        letterSpacing: -0.5, margin: '0 0 6px' }}>Hikoyangizni boshlang.</h1>
      <p style={{ fontFamily: 'var(--font-geist)', fontSize: 14, color: mute, margin: '0 0 22px' }}>
        Bepul, abadiy. O&apos;qing yoki yozing — o&apos;zingiz qaror qiling.
      </p>
      ...
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <input required placeholder="Ko‘rinadigan ism" value={name}
          onChange={e => setName(e.target.value)} style={inputStyle} />
        <input required placeholder="foydalanuvchi nomi (masalan: amara.yozadi)" value={username}
        ...
        <input type="password" required minLength={6} placeholder="Parol (kamida 6 belgi)" value={password}
      ...
      <p style={{ marginTop: 14, fontFamily: 'var(--font-geist)', fontSize: 11.5,
        ...
        Hisob yaratish bilan siz shartlar va maxfiylik siyosatiga rozilik bildirasiz.
      </p>
      <div style={{ marginTop: 16, textAlign: 'center', fontFamily: 'var(--font-geist)', fontSize: 13, color: mute }}>
        Hisobingiz bormi? <Link href="/auth/login" style={{ color: tokens.orange, fontWeight: 600, textDecoration: 'none' }}>Kirish</Link>
      </div>
    </div>
  );
}
